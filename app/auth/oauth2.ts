...
export function oauth2Start(provider: string) {
  location.href = `${GAIA_API_BASE_URI}/oauth2/start/personas/${provider}`;
}
...
export async function oauth2LoginWithIdToken(provider: string, idToken: string, nonce: string): Promise<{ ok: boolean; sessionId: string }> {
  const response = await fetch(`${GAIA_API_BASE_URI}/oauth2/login-with-idtoken/${provider}`, {
    method: 'POST',
    headers: {
...
export async function oauth2Me(): Promise<OAuth2MeResult> {
  const response = await fetch(`${GAIA_API_BASE_URI}/oauth2/me`, {
    headers: {
...
export async function oauth2MeByToken(provider: string): Promise<OAuth2MeResult> {
  const response = await fetch(`${GAIA_API_BASE_URI}/oauth2/me-by-token/${provider}`, {
    headers: {
...
export async function oauth2Logout() {
  await fetch(`${GAIA_API_BASE_URI}/oauth2/logout`, {
    method: 'POST',
    headers: {
      'Authorization': `Bearer ${sessionManager.get()}`,
      'Content-Type': 'application/json',
...
export async function oauthLinkWallet() {
  const response = await fetch(`${GAIA_API_BASE_URI}/oauth2/link-wallet`, {
    method: 'POST',
    headers: {
      'Authorization': `Bearer ${sessionManager.get()}`,
...
export async function oauthUnlinkWalletByToken() {
  await fetch(`${GAIA_API_BASE_URI}/oauth2/unlink-wallet-by-token`, {
    method: 'POST',
    headers: {
      'Authorization': `Bearer ${tokenManager.getToken()}`,
      'Content-Type': 'application/json',
...
export async function oauthUnlinkWalletBySession() {
  await fetch(`${GAIA_API_BASE_URI}/oauth2/unlink-wallet-by-session`, {
    method: 'POST',
    headers: {
      'Authorization': `Bearer ${sessionManager.get()}`,
      'Content-Type': 'application/json',
